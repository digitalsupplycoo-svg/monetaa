# Moneta — money calculators, guides and blog

Static site generated by `python3 build.py`. The blog pulls from Sanity at
build time, so Vercel now runs that build command instead of deploying the
folder as-is — see `DEPLOY.md` for the full setup, the Studio login, and how
publishing a post gets it live.
...
It also configures the daily cron that keeps the blog in sync — details in `DEPLOY.md`.
...
blog/                   generated from Sanity at build time — see DEPLOY.md
studio/                 Sanity Studio source (own npm project, deployed separately)
api/rebuild.js          hit by the daily cron to trigger a redeploy
about / contact / privacy / terms / disclaimer / 404
assets/                 base.css, site.js (motion), search.js, money.js + loan.js + income.js
